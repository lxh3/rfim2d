...
from .save_and_load import load_func, save_func, load_svA, load_hvdMdh
from .param_dict import split_dict, join_dict, get_keys, divvy_params
from .residuals import As_residual, dMdh_residual, Sigma_residual, eta_residual, joint_residual
...
def save_and_show(params, fit_type, func_type='wellbehaved', filename=None, show=True):
    """
    Save fit parameters and/or plot fit information in figure format
    """
    if filename != None:
        save_func(filename+'.pkl.gz',params)
        print_fit_info(params, fit_type, func_type=func_type, filename=filename+'.png', show=False)
...
def fit_As_Scaling(args, params0=None, filename=None, verbose=False, show_params=True):
    """
    ...
    Input:
        ...
        verbose - flag to print cost
        show_params - flag for whether to plot parameter names with their values
    Output:
...
def fit_dMdh_Scaling(args, params0=None, weight_list=None, filename=None, verbose=False, show_params=True):
    """
    ...
    params, err = perform_fit(dMdh_residual, params0, allargs, verbose=verbose)
...
def Sigma_fit(args, params0=None, sigmaNu_fixed=True, func_type='wellbehaved', filename=None, verbose=False, show_params=True):
    """
    ...
    params, err = perform_fit(Sigma_residual, params0, fullargs, verbose=verbose)
    ...
    save_and_show(params, 'Sigma', func_type=func_type, filename=filename, show=show_params)
...
def eta_fit(args, params0=None, func_type='wellbehaved', filename=None, verbose=False, show_params=True):
    """
    ...
    save_and_show(params, 'eta', func_type=func_type, filename=filename, show=show_params)

    return params, err


def joint_fit(args, params0=None, sigmaNu_fixed=True, func_type='wellbehaved', filename=None, verbose=False, show_params=True):
    """
    ...
    params, err = perform_fit(joint_residual, params0, fullargs, verbose=verbose)
    ...
    save_and_show(params, 'joint', func_type=func_type, filename=filename, show=show_params)
 
    return params, err


#### Perform all fits (A, dMdh, and Sigma and eta jointly) and return parameters

def perform_all_fits(filenames=[None,None], data=None, sigmaNu_fixed=True, func_type='wellbehaved', verbose=False, show_params=True):
    """
    ...
    args_A = [s, As]
    params_A, err = fit_As_Scaling(args_A, verbose=verbose, show_params=show_params)
    Sigma = params_A['Sigma']

    args_dMdh = [h,dMdh]
    params_dMdh, err = fit_dMdh_Scaling(args_dMdh, verbose=verbose, show_params=show_params)
    eta = params_dMdh['eta']

    args = [rA, Sigma, rdMdh, eta]
    params, err = joint_fit(args, sigmaNu_fixed=sigmaNu_fixed, func_type=func_type, verbose=verbose, show_params=show_params)
    params_Sigma, params_eta, prior = divvy_params(params, sigmaNu_fixed=sigmaNu_fixed, func_type=func_type)
