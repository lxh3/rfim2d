...
key_dict = {
    ...
    'joint': [
        'rScale', 'sScale', 'etaScale', 'sigmaNu',
    ...
    if fit_type in fit_types:
        if func_type == 'powerlaw':
            try:
                ...
                print('Error: '+fit_type+' does not have a powerlaw form. '
...
def format_for_residual(params_dict_Sigma, params_dict_eta, func_type):
    ...
    if func_type == 'powerlaw':
        ...
        pe = params_dict_eta
        priorWeight = 0.1
        prior = priorWeight*np.array([
                np.log(pe['rScale']), np.log(pe['etaScale']),
                np.log(pe['betaDeltaOverNu']), pe['B'], pe['C']
                ])
...
def divvy_params(params, sigmaNu_fixed=True, func_type='wellbehaved'):
    """
    divide parameters into those associated with Sigma(r)
    and those associated with eta(r)
    Input:
        params - list of all parameters found from joint fit
        sigmaNu_fixed - whether sigmaNu was set to 0.5 in fit
        func_type - form of dw/dl used - supplied to detemine
                    number of params for each function
    Output:
    ...
    if not isinstance(params, dict):
        params_Sigma, params_eta, prior = \
            format_for_residual(params_Sigma, params_eta, func_type)
    else:
        prior = None
