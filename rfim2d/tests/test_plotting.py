...
def test_plotting_svA():
    ...
    plotting.scatter(data,labels,logscale=logscale, Range=Range, figure_name=figure_name, show=False)
    return
...
def test_plotting_fits_svA():
    ...
    plotting.scatter_vs_function(data, function, labels, constant, constant_given_r = constant_given_r, logscale=logscale, Range=Range, figure_name=figure_name, show=False)
    return
...
def test_plotting_collapse_svA():
    ...
    plotting.collapse(data, function, labels, constant, constant_given_r, logscale=logscale, Range=Range, figure_name=figure_name, show=False)
    return
...
def perform_joint_fits(args, sigmaNu_fixed=True, func_type='wellbehaved'):
    params, err = fitting.joint_fit(args, sigmaNu_fixed=sigmaNu_fixed, func_type=func_type, show_params=False)
    params_Sigma, params_eta, prior = fitting.divvy_params(params, sigmaNu_fixed, func_type)
...
def test_plotting_joint_fits():
    ...
    for t in types:
        ...
        loc = 'upper right'
        plotting.compare(data_Sigma, scaling.Sigma_func, labels, params_Sigma, logscale=logscale, loc=loc, figure_name=figure_name, types=t, show=False)
        labels = [r'$r$',r'$\eta(r)$']
        logscale = [False,True]
        figure_name = None #'eta_'+t+'.png'
        loc = 'lower right'
        plotting.compare(data_eta, scaling.eta_func, labels, params_eta, logscale=logscale, loc=loc, figure_name=figure_name, types=t, show=False)
    return
...
def test_comparing_all_forms():
    ...
    logscale = [False,True]
    loc = 'upper right'
    plotting.compare(data_Sigma, scaling.Sigma_func, labels, pS, logscale=logscale, loc=loc, show=False)
    labels = [r'$r$',r'$\eta(r)$']
    logscale = [False,True]
    loc = 'lower right'
    plotting.compare(data_eta, scaling.eta_func, labels, pe, logscale=logscale, loc=loc, show=False)
    return
