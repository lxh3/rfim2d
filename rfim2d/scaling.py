...
def Sigma_functional_form(func_type='wellbehaved'):
    ...
    if func_type == 'powerlaw':
        ...
        form = (r'$\Sigma(w)'
                r'=\big{(}\frac{1}{w}+B\big{)}^{-B\frac{1}{\sigma\nu}+F}'
                r'exp\big{(}\frac{1}{w\sigma\nu}\big{)}$')
    elif func_type == 'wellbehaved':
        form = (r'$\Sigma(w)=w^{\frac{B}{\sigma\nu}-F}'
                r'exp\big{(}\frac{1}{w\sigma\nu}+B F w\big{)}$')
    elif func_type == 'pitchfork':
        form = (r'$\Sigma(w)=w^{\frac{B}{\sigma\nu}}'
                r'(1+Bw^2)^{-\frac{B}{2\sigma\nu}}'
                r'exp\big{(}\frac{1}{2w^2\sigma\nu}+\frac{F}{w}'
                r'+\sqrt{B}FArcTan[\sqrt{B}w]\big{)}$')
    else:
...
def Sigma_func(r_list, constant, func_type='wellbehaved', scaled=False):
    ...
    Input:
        ...
        scaled - flag for whether to use the Sigma derived with the
                 'simple' dw/dl scaled for comparison with the
                 'wellbehaved' form at small r
    Output:
    ...
    if func_type == 'powerlaw':
        ...
        if func_type == 'simple':
            Sigma = (abs(sScale)
                     * np.exp(1/(w*sigmaNu)) * (B+(1/w)) ** (F-(B/sigmaNu)))
            if scaled:
                Sigma = np.exp((B**2*w)/sigmaNu)*Sigma
            return Sigma
        elif func_type == 'wellbehaved':
            Sigma = (abs(sScale)
                     * np.exp(B*F*w + 1/(w*sigmaNu))
                     * w ** ((B/sigmaNu)-F))
            if scaled:
                Sigma = np.exp((-B**2*w)/sigmaNu)*Sigma
            return Sigma
        elif func_type == 'pitchfork':
            exponential_piece = (np.exp((F/w)+(1/(2*sigmaNu*w**2))
            ...
            print("functional type not recognized,",
                  " using 'wellbehaved' form")
            Sigma = (abs(sScale) * np.exp(B*F*w + 1/(w*sigmaNu))
                     * w ** ((B/sigmaNu)-F))
            return Sigma
...
def eta_functional_form(func_type='wellbehaved'):
    ...
    if func_type == 'powerlaw':
        ...
        form = (r'$\eta(w)='
                r'\big{(}\frac{1}{w}+B\big{)}^{B\frac{\beta\delta}{\nu}-C}'
                r'exp\big{(}-\frac{\beta\delta}{w\nu}\big{)}$')
    elif func_type == 'wellbehaved':
        form = (r'$\eta(w) = w^{-B\frac{\beta\delta}{\nu}+C}'
                r'exp\big{(}-\frac{\beta\delta}{\nu}\frac{1}{w}'
                r'-BCw\big{)}$')
    elif func_type == 'pitchfork':
        form = (r'$\eta(w)=w^{-B\frac{\beta\delta}{\nu}}'
                r'(1+Bw^2)^{\frac{B\beta\delta}{2\nu}}'
                r'exp\big{(}-\frac{\beta\delta}{2w^2\nu}'
                r'-\frac{C}{w}-\sqrt{B}CArcTan[\sqrt{B}w]\big{)}$')
    else:
        print('functional form requested not recognized')
        form = 'unknown functional form'
    return form
...
def eta_func(r_list, constant, func_type='wellbehaved', scaled=False):
    ...
    if func_type == 'powerlaw':
        ...
        w = r/rScale
        if func_type == 'simple':
            eta = (etaScale*np.exp(-betaDeltaOverNu/w)
                   * (B+(1/w)) ** (-C+B*betaDeltaOverNu))
            if scaled:
                eta = np.exp(-B**2*w*betaDeltaOverNu)*eta
            return eta
        elif func_type == 'wellbehaved':
            eta = (etaScale*np.exp(-B*C*w-betaDeltaOverNu/w)
                   * w ** (-B*betaDeltaOverNu+C))
            if scaled:
                eta = np.exp(B**2*w*betaDeltaOverNu)*eta
            return eta
        elif func_type == 'pitchfork':
            exponential_piece = (np.exp(-(C/w)-(betaDeltaOverNu/(2*w**2))
                                 - np.sqrt(B) * C * np.arctan(np.sqrt(B)*w)))
            powerlaw_piece = (w**(-B*betaDeltaOverNu)
                              * (1+B*w**2) ** (B*betaDeltaOverNu/2))
